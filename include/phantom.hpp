...
public:
    ...
    /// @param out A pointer to n vectors of key shares that are produced
    ...
    virtual int32_t create(phantom_vector<uint8_t> *out, const uint8_t *key, size_t n, size_t k) = 0;

    /// @brief Consume an array of shared keys and produce the secret key
    /// @param obj The key sharing context
    /// @param key The secret key to be produced
    /// @param out A pointer to k vectors of key shares that are consumed
    /// @param k The minimum number of users required to retrieve the secret key
    /// @return EXIT_SUCCESS upon success, otherwise EXIT_FAILURE
    virtual int32_t combine(uint8_t *key, const phantom_vector<uint8_t> *shares, size_t k) = 0;
