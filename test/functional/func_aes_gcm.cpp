...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i < 3; i++) {
        ...
        symmetric_key_type_e key_type = (0 == i) ? SYMKEY_AES_128_GCM :
                                        (1 == i) ? SYMKEY_AES_192_GCM
                                                 : SYMKEY_AES_256_GCM;
        ...
        for (size_t j=0; j < NUM_ITER/64; j++) {
            ...
            phantom_vector<uint8_t> pt(num_bytes), ct(num_bytes), rt(num_bytes), ctr(12);
            ...
            for (size_t k=0; k < 64; k++) {
                ...
                symmetric_key_cipher::encrypt_start(aesenc.get(), ctr.data(), 12);
            ...
            for (size_t k=0; k < 64; k++) {
                ...
                symmetric_key_cipher::decrypt_start(aesdec.get(), ctr.data(), 12);
                symmetric_key_cipher::decrypt(aesdec.get(), rt.data(), ct.data(), num_bytes);
                symmetric_key_cipher::decrypt_finish(aesdec.get(), recovered_tag.data(), 12);
            }
            sw_decrypt.stop();
        ...
        std::cout << "AES-GCM length=" << static_cast<int>(num_bytes) << std::endl;
