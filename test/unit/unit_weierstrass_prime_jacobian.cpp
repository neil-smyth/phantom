...
const lest::test specification[] =
{
    CASE("Projective point - 32-bit")
    {
        ...
        weierstrass_prime_jacobian<uint32_t> p(cfg, x, y);
        EXPECT(p.x() == x);
    ...
    CASE("Projective point addition and subtraction zero x - 32-bit")
    {
        ...
        weierstrass_prime_jacobian<uint32_t> p1(cfg, x1, y1);
        weierstrass_prime_jacobian<uint32_t> p2(cfg, x1, y1);
        EXPECT(p1.x() == x1);
        EXPECT(p1.y() == y1);
    ...
    CASE("Projective point addition non-zero x - 32-bit")
    {
        ...
        weierstrass_prime_jacobian<uint32_t> p1(cfg, x1, y1);
        weierstrass_prime_jacobian<uint32_t> p2(cfg, x2, y2);
        weierstrass_prime_jacobian<uint32_t> pref = p1;
        EXPECT(p1.x() == x1);
        EXPECT(p2.x() == x2);
    ...
    CASE("Projective point (Montgomery) - 64-bit")
    {
        ...
        weierstrass_prime_jacobian<uint64_t> p(cfg, x, y);
        mpz<uint64_t> xr, yr;
        p.convert_from(cfg, &xr, &yr);
        EXPECT(xr.get_str(16) == x.get_str(16));
        EXPECT(yr.get_str(16, true) == "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFFFFFFFFFFFE");
