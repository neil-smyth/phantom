...
json perf_ibe::run(phantom::pkc_e pkc_type, size_t duration_us, cpu_word_size_e size_hint, bool masking)
{
    std::cout << "  PKC :: IBE :: DLP :: " << static_cast<int>(size_hint) << "-bit :: " <<
    ...
    do {
        ...
        ctx_pkg = ibe_dlp_a.create_ctx(param_set, size_hint, masking);
        ...
        std::unique_ptr<user_ctx> ctx_client = ibe_dlp_a.create_ctx(param_set, size_hint, masking);
        std::unique_ptr<user_ctx> ctx_server = ibe_dlp_b.create_ctx(param_set, size_hint, masking);
