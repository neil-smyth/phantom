...
bool kyber_pke::keygen(std::unique_ptr<user_ctx>& ctx)
{
    ...
    myctx.pke()->keygen(myctx.rho(), myctx.s().data(), myctx.t().data());
...
bool kyber_pke::set_public_key(std::unique_ptr<user_ctx>& ctx, const phantom_vector<uint8_t>& k)
{
    ...
    myctx.t() = phantom_vector<int16_t>(n);
            ...
            phantom_vector<uint8_t>& ct)
{
    ...
    int16_t *v = u + k * n;
    myctx.pke()->enc(u, v, myctx.t_ntt().data(), myctx.rho(), n_bits, k, pt.data());
            ...
            phantom_vector<uint8_t>& pt)
{
    ...
    myctx.pke()->dec(u, v, us, n_bits, k, m);
