...
/// Definitions for the eddsa parameter sets
template<class T>
struct eddsa_set_t
{
    uint16_t                            set;
    ...
    eddsa_set_t() {}
    virtual ~eddsa_set_t() {}
    eddsa_set_t(eddsa_set_t&&) = default;
};

/**
 * @brief An EdDSA context class used to store key, curve and algorithm specific parameters
 */

class ctx_eddsa : public user_ctx
{
public:
    virtual ~ctx_eddsa() {}
    ...
    virtual phantom_vector<uint8_t>& prefix() = 0;
    virtual size_t n() = 0;
...
/// A class describing the eddsa user context
template<class T>
class ctx_eddsa_tmpl : public ctx_eddsa
{
public:
    explicit ctx_eddsa_tmpl(size_t set) : m_scheme(PKC_SIG_EDDSA), m_set(set)
    {
        m_params.set = set;
        switch (set)
        {
            case 0:
            case 1:
            case 2:  m_params.curve = &elliptic::curves::param_ec_edwards25519;
                     m_params.field = elliptic::field_e::EDWARDS_PRIME_FIELD;
                     break;
            case 3:
            case 4:  m_params.curve = &elliptic::curves::param_ec_edwards448;
                     m_params.field = elliptic::field_e::EDWARDS_PRIME_FIELD;
                     break;
        ...
        {
            case elliptic::field_e::EDWARDS_PRIME_FIELD:  prime_setup();  break;
            default: {}
        }
    ...
    virtual ~ctx_eddsa_tmpl() {}

    ctx_eddsa_tmpl(const ctx_eddsa_tmpl&) = delete;
    ...
    virtual phantom_vector<uint8_t>& prefix() { return m_prefix; }
    virtual phantom_vector<uint8_t>& pk() { return m_pk; }
    virtual phantom_vector<uint8_t>& A() { return m_A; }
    ...
    {
        auto p = const_cast<elliptic::point<T>*>(m_eddsa_pk->get_point());
        p->convert_to_mixed(m_params.cfg);
        return *p;
    }
    ...
    void set_s(const phantom_vector<uint8_t>& bytes)
    {
        m_s.set_bytes(bytes);
    }

    void set_prefix(uint8_t* prefix, size_t n)
    {
        m_prefix = phantom_vector<uint8_t>(prefix, prefix + n);
    }

    void set_A(const phantom_vector<uint8_t>& bytes)
    {
        m_A = phantom_vector<uint8_t>(bytes.begin(), bytes.end());
    }
...
private:
    void prime_setup()
    {
        ...
        m_params.cfg.a_is_minus_1 = m_set < 3;

        core::mpz<T> g_x(m_params.curve->g_x, 16);
        core::mpz<T> g_y(m_params.curve->g_y, 16);
        m_params.base = std::unique_ptr<elliptic::point<T>>(
            new elliptic::edwards_prime_affine<T>(m_params.cfg, g_x, g_y));

        m_eddsa = std::unique_ptr<elliptic::ecc<T>>(
    ...
    std::unique_ptr<elliptic::ecc<T>> m_eddsa;
    std::unique_ptr<elliptic::ecc<T>> m_eddsa_pk;
    ...
    core::mpz<T> m_s;
    phantom_vector<uint8_t> m_prefix;
    phantom_vector<uint8_t> m_sk;
    phantom_vector<uint8_t> m_pk;
    phantom_vector<uint8_t> m_A;
};
